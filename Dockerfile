...
# docker run -it --rm -v $(pwd):/build ghcr.io/jniltinho/rust-cross-mac bash
...
RUN apt-get update -qq \
    && apt-get install -yqq llvm-dev libclang-dev pkg-config clang libxml2-dev make cmake curl git file libz-dev zlib1g-dev libssl-dev gcc-mingw-w64-x86-64 \
    && apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* /var/cache/apt/archive/*.deb

COPY files/compile-bin.tar.gz /opt/
COPY cargo_config /opt/osxcross/
